...
private:
    ...
    ros::Publisher pubGlobalPathRough;
...
public:
    TraversabilityPRM():
        ...
        subGoal = nh.subscribe<geometry_msgs::PoseStamped>("/move_base_simple/goal", 5, &TraversabilityPRM::goalPosHandler, this);
        ...
        pubGlobalPath = nh.advertise<nav_msgs::Path>("/global_path", 5);//平滑后
        pubGlobalPathRough = nh.advertise<nav_msgs::Path>("/global_path_rough", 5);//不平滑
    ...
    void buildRoadMap(){
        ...
        //     std::cout << nodeList[i]->x[0] << " " << nodeList[i]->x[1] << std::endl;
        // }
        // 3. Planning
        bfsSearch();
        // 4. Visualize Roadmap
    ...
    void smoothPath(){
        ...
        for (int i = 0; i < pathList.size(); i++){
            ...
            std::cout << "path: " << pathList[i]->x[0] << " " << pathList[i]->x[1] << " "
                << pathList[i]->x[2] << std::endl;
        }
    ...
    void publishGlobalPathRough(){
        ...
        pubGlobalPathRough.publish(originPath);
